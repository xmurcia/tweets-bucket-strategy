...
const ELON_POSTS_CACHE_TTL_MS = 60 * 1000;
const ELON_AUTHOR_CACHE_TTL_MS = ELON_POSTS_CACHE_TTL_MS;
...
type ElonPostsResponsePayload = {
  author: ElonPostAuthor;
  items: ElonPostResponseItem[];
};
...
const elonPostsCache = new Map<string, { expiresAt: number; payload: ElonPostsResponsePayload }>();
let elonAuthorCache: { expiresAt: number; payload: ElonPostAuthor } | null = null;
...
function getFirstNonEmptyString(...candidates: unknown[]): string | null {
...
function normalizeHandle(rawHandle: string | null): string {
  if (!rawHandle) return DEFAULT_ELON_AUTHOR.handle;
  return rawHandle.replace(/^@+/, '').trim() || DEFAULT_ELON_AUTHOR.handle;
}

function extractElonAuthorFromXtracker(payload: unknown): ElonPostAuthor {
  ...
  const name = getFirstNonEmptyString(
    ...
    user?.name,
    user?.displayName,
    DEFAULT_ELON_AUTHOR.name,
  ) ?? DEFAULT_ELON_AUTHOR.name;

  const handle = normalizeHandle(getFirstNonEmptyString(
    data?.handle,
    data?.username,
    data?.userName,
    profile?.handle,
...
async function getElonAuthorMeta(xtrackerApiUrl: string): Promise<ElonPostAuthor> {
  if (elonAuthorCache && elonAuthorCache.expiresAt > Date.now()) {
    return elonAuthorCache.payload;
  }

  try {
    const userRes = await fetch(`${xtrackerApiUrl}/users/elonmusk`, { headers: BROWSER_HEADERS });
    if (!userRes.ok) {
      throw new Error(`Xtracker user status ${userRes.status}`);
    }

    const userJson = await userRes.json() as unknown;
    const author = extractElonAuthorFromXtracker(userJson);

    elonAuthorCache = {
      expiresAt: Date.now() + ELON_AUTHOR_CACHE_TTL_MS,
      payload: author,
    };

    return author;
  } catch (error) {
    console.warn('Elon author metadata fetch failed:', error);
...
async function startServer() {
  ...
  app.get('/api/polymarket/token-quotes', async (req, res) => {
    const tokenIdsParam = typeof req.query.tokenIds === 'string' ? req.query.tokenIds : '';
    ...
    if (tokenIds.length > 120) {
      return res.status(400).json({ error: 'too many tokenIds (max 120)' });
    }

    const fetchPrice = async (tokenId: string, side: 'buy' | 'sell'): Promise<number | undefined> => {
      try {
        const response = await fetch(
          `${CLOB_API_URL}/price?token_id=${encodeURIComponent(tokenId)}&side=${side}`,
    ...
    try {
      const quoteEntries = await Promise.all(tokenIds.map(async (tokenId) => {
        // CLOB /price side=sell corresponds to taking the ask (buying shares).
        const [ask, bid] = await Promise.all([
          fetchPrice(tokenId, 'sell'),
          fetchPrice(tokenId, 'buy'),
        ]);

        const spread = ask !== undefined && bid !== undefined
          ? (ask - bid) * 100
          : undefined;

        return [tokenId, { ask, bid, spread }] as const;
      }));

      res.json({ quotes: Object.fromEntries(quoteEntries) });
    } catch (error) {
      console.error('Token quotes proxy error:', error);
      res.status(500).json({ error: 'Failed to fetch token quotes' });
    }
  });

  app.get('/api/polymarket/elon-posts', async (req, res) => {
    try {
      ...
      if (startDate && Number.isNaN(new Date(startDate).getTime())) {
        return res.status(400).json({ error: 'startDate must be a valid ISO date string' });
      }
      ...
      if (rawLimit !== undefined) {
        ...
        if (Number.isNaN(parsed)) {
          return res.status(400).json({ error: 'limit must be an integer between 1 and 30' });
        }
        limit = Math.max(1, Math.min(30, parsed));
      }

      const cacheKey = `${startDate ?? ''}|${endDate ?? ''}|${limit}`;
      const cached = elonPostsCache.get(cacheKey);
      ...
      const upstreamUrl = `${XTRACKER_API_URL}/users/elonmusk/posts${params.toString() ? `?${params.toString()}` : ''}`;
      const [upstreamResponse, author] = await Promise.all([
        fetch(upstreamUrl, { headers: BROWSER_HEADERS }),
        getElonAuthorMeta(XTRACKER_API_URL),
      ]);

      if (!upstreamResponse.ok) {
        return res.status(502).json({ error: `Failed to fetch posts from XTracker (status ${upstreamResponse.status})` });
      }

      const upstreamJson = await upstreamResponse.json() as {
        data?:
          | Array<{ platformId?: string | number; content?: string; createdAt?: string }>
          | {
            posts?: Array<{ platformId?: string | number; content?: string; createdAt?: string }>;
          };
        posts?: Array<{ platformId?: string | number; content?: string; createdAt?: string }>;
      };

      const rawItems = Array.isArray(upstreamJson?.data)
        ? upstreamJson.data
        : upstreamJson?.data?.posts ?? upstreamJson?.posts ?? [];

      const items = rawItems
        .filter((item) => {
          const hasPlatformId = item?.platformId !== undefined && item?.platformId !== null && String(item.platformId).trim().length > 0;
          const hasContent = typeof item?.content === 'string' && item.content.trim().length > 0;
          return hasPlatformId && hasContent;
        })
        .map((item) => {
          const id = String(item.platformId).trim();
          return {
            id,
      ...
      const payload: ElonPostsResponsePayload = { author, items };

      elonPostsCache.set(cacheKey, {
        expiresAt: Date.now() + ELON_POSTS_CACHE_TTL_MS,
        payload,
      });

      res.json(payload);
    } catch (error) {
      console.error('Elon posts proxy error:', error);
      res.status(500).json({ error: 'Failed to fetch Elon posts feed' });
    }
  });
